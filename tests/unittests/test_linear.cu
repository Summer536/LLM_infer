#include <algorithm>  
#include <iostream>   
#include <math.h>   
#include <stdlib.h>    
#include <string>      
#include <vector>      
#include <stdio.h>
#include <fstream>

#include "src/utils/macro.h"
#include "src/weights/base_weights.h"
#include "src/kernels/linear.h"

void CPUlinear(float *input,float *weight, float *output,
               int m, int k, int n) {
    for(int i = 0; i < m; i++){
        for(int j = 0; j < n; j++){
            float sum = 0.0f;
            for(int l = 0; l < k; l++){
                sum += input[i * k + l] * weight[l * n + j];
            }
            output[i * n + j] = sum;
        }
    }
}

bool CheckResult(float *CPUoutput, float *GPUoutput, int output_size) {
    for(int i = 0; i < output_size; i++){
        if(i < 5){
            printf("0th result %d: CPU = %f, GPU = %f\n", i, CPUoutput[i], GPUoutput[i]);
        }
        if(fabs(CPUoutput[i] - GPUoutput[i]) > 1e-3){
            printf("Mismatch at %d, CPU = %f, GPU = %f\n", i, CPUoutput[i], GPUoutput[i]);
            return false;
        }
    }
    return true;
}

int main(int argc, char *argv[]){
    const int seqlen = 13;
    const int hidden_units = 4096;
    const int vocab_size = 32;
    const int inter_size = 10;
    int hidden_units_2 = 0;
    int output_size = 0;

    hidden_units_2 = hidden_units * hidden_units;
    output_size = seqlen * hidden_units;

    float *h_w = (float *)malloc(hidden_units_2 * sizeof(float));
    float *d_w;
    cudaMalloc(&d_w, hidden_units_2 * sizeof(float));

    for(int i = 0; i < hidden_units_2; i++){
        h_w[i] = (float)(i % 3);
    }

    float* h_in = (float*) malloc(sizeof(float) * hidden_units * seqlen);
    float* d_in;
    cudaMalloc((void**)&d_in, sizeof(float) * seqlen *  hidden_units);
    for(int i = 0; i < hidden_units * seqlen; i++) {
       h_in[i] = (float)(i % 3);
    }

    float *h_out = (float *)malloc(output_size * sizeof(float));
    float *d_out;
    cudaMalloc(&d_out, output_size * sizeof(float));
    
    CHECK(cudaMemcpy(d_w, h_w, hidden_units_2 * sizeof(float), cudaMemcpyHostToDevice));
    CHECK(cudaMemcpy(d_in, h_in, sizeof(float) * seqlen * hidden_units, cudaMemcpyHostToDevice));

    DataType type = getTensorType<float>();
    WeightType wtype = getWeightType<float>();
    
    TensorWrapper<float> *in = new TensorWrapper<float>(Device::GPU, type, {seqlen, hidden_units}, d_in);
    BaseWeight<float> weight;
    weight.shape = {hidden_units, hidden_units};
    weight.data = d_w;
    weight.type = wtype;

    TensorWrapper<float> *out = new TensorWrapper<float>(Device::GPU, type, {seqlen, hidden_units}, d_out);

    cublasHandle_t cublas_handle;
    cublasLtHandle_t cublaslt_handle;
    cublasCreate(&cublas_handle);
    cublasSetMathMode(cublas_handle, CUBLAS_DEFAULT_MATH);
    cublasWrapper *cublas_wrapper = new cublasWrapper(cublas_handle, cublaslt_handle);
    cublas_wrapper->setFP32GemmConfig();

    std::cout << "before launch linear gemm kernel" << std::endl;
    launchLinearGemm(in, weight, out, cublas_wrapper);
    std::cout << "after launch linear gemm kernel" << std::endl;

    CHECK(cudaMemcpy(h_out, d_out, output_size * sizeof(float), cudaMemcpyDeviceToHost));
    float* CPUout = (float*) malloc(output_size * sizeof(float));
    CPUlinear(h_in, h_w, CPUout, seqlen, hidden_units, hidden_units);

    bool check_result = CheckResult(CPUout, h_out, output_size);
    if(check_result){
        std::cout << "Test passed" << std::endl;
    }

    free(h_w);
    free(h_in);
    free(h_out);
    free(CPUout);
    cudaFree(d_w);
    cudaFree(d_in);
    cudaFree(d_out);
}

