...
template <typename T>
__global__ void BuildCausalMasksConsideringContextPastKV(T*mask, const int*q_lens, const int*k_lens, int max_q_len, int max_k_len){
    int tid = threadIdx.x;
...
void launchBuildCausalMasks(TensorWrapper<T>*mask,
    ...
    BuildCausalMasksConsideringContextPastKV<<<batch_size, 256>>>(mask->data, q_lens->data, k_lens->data, max_q_len, max_k_len);
}
