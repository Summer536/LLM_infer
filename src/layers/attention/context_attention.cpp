...
template<typename T> 
LLaMAContextAttentionLayer<T>::LLaMAContextAttentionLayer( //.h中定义的初始化函数
                               int head_num,
                               int kv_head_num,
                               int head_size,
...
///////////////////////////////分配中间buffer的函数///////////////////////////
template<typename T>    
void LLaMAContextAttentionLayer<T>::allocForForward(LLaMAAttentionDynParams& params) { //LLaMAAttentionDynParams:(位于/src/models/llama/llama_params.h)
    ...
    /////////调用allocator->Malloc 分配buffer//////////////////////////////////
    qkv_buf_wo_pad->data = allocator->Malloc(qkv_buf_wo_pad->data, sizeof(T) * num_tokens * qkv_head_num * head_size, false);

    q_buf_w_pad->data = allocator->Malloc(q_buf_w_pad->data, sizeof(T) * qkv_head_num * batch_size * max_q_len * head_size, false);
...
void LLaMAContextAttentionLayer<T>::freeBuf(){
    ...
    DeviceSyncAndCheckCudaError();
    //no need to free v cache buf, because its included in k cache buf->data
...
void LLaMAContextAttentionLayer<T>::forward(TensorMap& inputs, TensorMap& outputs, LLaMAattentionWeights<T>& weights, LLaMAAttentionDynParams& params, LLaMAAttentionStaticParams& static_params)
{
    ...
    //第一个参数：请注意我们刚刚获取到的是一个（Tensor*），但是该函数要求TensorWrapper<T>* ，因此这里做了一个->as<T>()的强转，这个强转是在 (位于/src/utils/tensor.h)中定义的
    ...
    // 8.transpose+reshape(shape:[bs,head,seqlen,headsize]=>[bs,seqlen,head,headsize]=>[numtokens,hiddenunits])+remove padding
...
template class LLaMAContextAttentionLayer<float>;//实例化
